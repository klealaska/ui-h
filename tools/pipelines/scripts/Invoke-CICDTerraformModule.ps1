...
# Check the RUNREMOVE environment variable and conditionally run Remove-TerraformWorkspace
 if ($env:RUNREMOVE -eq 'True') {
    ...
    Write-Verbose "Executing Invoke-TerraformPlan"
    Invoke-TerraformPlan -VarFile $VarFile -WorkingDirectory $WorkingDirectory -OptionList @("-refresh=$refreshValue")

    Write-Verbose "Executing Invoke-TerraformApply"
    Invoke-TerraformApply -VarFile $VarFile -WorkingDirectory $WorkingDirectory -Force -OptionList @("-refresh=$refreshValue")
 }

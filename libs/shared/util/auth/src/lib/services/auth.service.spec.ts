...
describe('AuthService', () => {
  ...
  beforeEach(() => {
    ...
    service = new AuthService(routerStub as any, httpStub as any);
  });
  ...
  describe('getAvidAuthLoginUrl', () => {
    it('should return login url', () =>
      expect(
        ...
        'https://login.qa.avidsuite.com/auth/login?to_app=DataCapture&sso_callback=http://localhost/sso/callback&to_url=http://localhost/'
      ));
  });
    ...
    describe('requested_url tests', () => {
      beforeEach(() => {
        Object.defineProperty(window, 'location', {
          value: new URL('http://localhost/queue'),
        });
        jest.spyOn(window.sessionStorage.__proto__, 'setItem');
        window.sessionStorage.__proto__.setItem = jest.fn().mockImplementationOnce(() => tokens);
      });
      ...
      it('should redirect to requested_url', () => {
        httpStub.post.mockReturnValueOnce(
          of({
            return_data: {
              requested_url: 'http://localhost/research',
              tokens: JSON.parse(tokens),
            },
          } as TokenResponse)
        );
        ...
        expect(httpStub.post).toHaveBeenCalled();
        expect(sessionStorage.setItem).toHaveBeenCalledWith('tokens', tokens);
        expect(window.location.href).toEqual('http://localhost/research');
      });

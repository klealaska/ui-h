...
describe('XdcService', () => {
  ...
  it('should call the submit indexed api', done => {
  ...
  it('should post for mass escalation', done => {
    const batchEscalationStub: BatchEscalationRequest = {
      docIds: ['1'],
      escalation: getEscalationStub(EscalationCategoryTypes.RecycleBin),
    };

    xdcService.postMassEscalation(batchEscalationStub).subscribe(response => {
      expect(response).toBeDefined();
      done();
    });

    const req = httpMock.expectOne(
      `${environmentStub.apiBaseUri}${XdcApiUrls.POST_INDEXED_MASS_ESCALATION}`
    );
    expect(req.request.method).toEqual('POST');

    req.flush(batchEscalationStub);
  });
  ...
  describe('getSkipUnindexedDocument()', () => {
    const bodyRequestMock = getAggregateBodyRequest({ buyerId: '25' } as any);
    ...
    it('should post to get the next unindexed document when skipping to xdc service with all params', done => {
      xdcService
        .getSkipUnindexedDocument('mockUsername', '25', SkipDocumentDirection.Next, bodyRequestMock)
        .subscribe(res => {
          expect(res).toEqual(compositeDataStub);
          done();
        });
      ...
      const req = httpMock.expectOne(
        `${parseUrlParams(`${environmentStub.apiBaseUri}${XdcApiUrls.GET_SKIP_DOCUMENT}`, {
          username: 'mockUsername',
          currentDocumentId: '25',
          skipDirection: SkipDocumentDirection.Next,
  ...
  describe('postArchive', () => {
    ...
    it('should get archived documents', done => {
      xdcService.postArchive(bodyMock).subscribe(response => {
        expect(response).toEqual(documentStub);
        done();
      });

      const req = httpMock.expectOne(`${environmentStub.apiBaseUri}${XdcApiUrls.POST_ARCHIVE}`);
      expect(req.request.method).toEqual('POST');
      req.flush(documentStub);
    });
  });
  ...
  describe('getAllPendingDocuments', () => {
    it('should get all pending upload documents', done => {
      xdcService.getAllPendingDocuments('mockUsername').subscribe(response => {
        expect(response).toBeDefined();
      ...
      const req = httpMock.expectOne(
        `${environmentStub.apiBaseUri}${XdcApiUrls.GET_ALLPENDINGUPLOADS}?username=mockUsername`
      );
      expect(req.request.method).toEqual('GET');
  ...
  describe('postCreatePendingUpload', () => {
    const pendingUploadDocumentsStub: PendingUploadDocument[] = [
      {
        correlationId: '1',
        buyerId: '1',
        username: 'mock',
    ...
    it('should post new pending upload documents', done => {
      xdcService.postCreatePendingUpload(pendingUploadDocumentsStub).subscribe(response => {
        expect(response).toBeDefined();
        done();
      });
      ...
      req.flush(pendingUploadDocumentsStub);
    });
  });
});

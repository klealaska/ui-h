...
import { createInbox, saveAttachment, sendEmail, uploadAttachment } from '../../support/email';
...
describe('The Motherload Test for an Indexer', () => {
  ...
  it('save attachment', () => saveAttachment().then(fileContent => (base64File = fileContent)));

  it('upload attachment', () =>
    uploadAttachment(base64File, newGuid).then(
      attachment => (attachmentId = attachment[0].toString())
    ));

  it('should create inbox', () => createInbox().then(inbox => (inboxId = inbox.id)));

  it('should send email', () => sendEmail('avidxchange@ft.avidbill.com', inboxId, attachmentId));

...
const fileName = `avidxchange_motherload-${newGuid}.pdf`;

describe('Start Scan', () => {
  beforeEach('Navigate to Scan', () => {
    cy.visit('/');
    cy.visit(Cypress.env('scanUrl'));
    cy.origin(Cypress.env('scanUrl'), () => {
      // do nothing
    });
  });

  it('should login', () => {
    cy.get('[id="user"]').type('PlatformScanServiceDev');
    cy.get('[id="pword"]').type('xEpe*EsAdruprAsPestlM6');
    cy.get('[id="btn-LoginButton"]').click();
    cy.get('[id="WCNew-OptBox"]').click();
    cy.get('[id="btn-PlacementOKButton"]').click();

    cy.wait(1000);
    cy.get(`[data-name="${Cypress.env('envAlias')}"]`).dblclick();
    cy.wait(200);
    cy.get('[data-name="OOPTestPartnerA"]').dblclick();
    cy.wait(200);
    cy.get('[data-name="ABN"]').dblclick();
    cy.wait(200);
    ...
    cy.request({
      method: 'GET',
      url: Cypress.env('scanHttpTriggerUrl'),
    }).then(response => {
...
describe('Return to localhost', () => {
  it('should go back to localhost', () => {
    cy.visit('/');
  });
});
...
describe('The Motherload Test for an Indexer', () => {
  ...
  it('02-open document & submit as duplicate with new cust acct #', () => {
    cy.visit('queue');
    cy.wait(5000);
    cy.contains('Delivery date', { timeout: 50000 }).click();
    cy.then(() => cy.wrap(MotherloadHelpers.openDocument('queue', newGuid)));
